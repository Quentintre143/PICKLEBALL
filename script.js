document.addEventListener('DOMContentLoaded', function () {
  ...
  function computeStatsFromEntries(entries) {
    var stats = { 'very-interested': 0, 'interested': 0, 'maybe': 0, 'not-interested': 0 };
    entries.forEach(function (e) {
      if (e && stats.hasOwnProperty(e.interest)) {
        stats[e.interest]++;
      }
    });
    return stats;
  ...
  // Récupère TOUTES les réponses (tous appareils confondus) depuis JSONBin.
  // Retourne null si JSONBin n'est pas configuré ou en cas d'erreur (on retombe
  // alors sur le cache local le plus récent).
  function fetchGlobalEntries() {
    if (!JSONBIN_READY) return Promise.resolve(null);
    return fetch(JSONBIN_BASE + '/latest', {
      headers: { 'X-Master-Key': JSONBIN_API_KEY }
    })
      .then(function (res) {
      ...
      .then(function (data) {
        return (data && data.record && Array.isArray(data.record.reponses)) ? data.record.reponses : [];
      })
      .catch(function (err) {
        console.error('Impossible de récupérer les statistiques globales (JSONBin) :', err);
        return null;
      });
  }
  ...
  // On affiche d'abord le cache local (rapide), puis on va chercher les vraies
  // statistiques globales (tous appareils confondus) sur JSONBin et on met à jour
  // l'affichage dès qu'elles arrivent.
  fetchGlobalEntries().then(function (entries) {
    if (entries === null) return; // JSONBin non configuré ou hors-ligne : on garde le cache local
    currentStats = computeStatsFromEntries(entries);
    currentVoters = entries.slice().reverse().slice(0, 200); // plus récent en premier
